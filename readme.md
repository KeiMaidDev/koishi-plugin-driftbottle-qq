...
- 扔漂流瓶的内容、配图和标题提示使用 Markdown + Keyboard；可直接跳过配图/标题或取消操作，完成后提供继续扔瓶、捞瓶和返回菜单按钮。
...
- 插件使用 `image-size` 读取图片的原始宽高；瓶子正文图片最大为 `1024 x 1024`，留言图片最大为 `600 x 600`，超过对应边界时等比例缩小。
- 图片下载或尺寸读取失败时，正文 Markdown 图片回退为 `1024 x 1024`，留言图片回退为 `600 x 600`，不影响漂流瓶内容发送。
...
- 举报数达到阈值后，插件会使用当前机器人向 `adminQQ` 中的管理员发送主动私聊审核通知。QQ 平台的 `adminQQ` 必须填写 `adapter-qq-crack` 提供的用户 OpenID（即该管理员触发消息时的 `session.userId`），不能填写纯数字 QQ 号；QQ 号无法反向转换为 OpenID。
